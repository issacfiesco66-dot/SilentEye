 ...
 */
const pushCircuitBreaker = new CircuitBreaker('web-push', 3, 60_000);

/**
 * Send push notification to specific users (admins + nearby users).
 * If userIds is empty, sends to ALL subscriptions (fallback for admins).
 * Protected by CircuitBreaker: 3 consecutive failures → 60s cooldown.
 */
export async function sendPushToUsers(userIds: string[], payload: PushPayload): Promise<number> {
  if (!VAPID_PUBLIC_KEY || !VAPID_PRIVATE_KEY) return 0;

  return pushCircuitBreaker.tryExecute(async () => {
    let result;
    if (userIds.length === 0) {
