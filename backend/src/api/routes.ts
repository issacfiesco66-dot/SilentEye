...
import {
  validateFieldReportInput,
  validateMissingPersonInput,
  isValidFieldReportStatus,
  isValidProfileMatchType,
  isValidProfileMatchConfidence,
  isValidNotificationChannel,
  isWithinMediaSizeLimit,
  redactCredentialMedia,
  type FieldReportMedia,
} from '../services/field-registry-service.js';
...
interface Permissions {
  ...
  /** Field verification registry (hallazgos de campo) — Phase 2 */
  ...
  switch (role) {
    case 'admin':
      return {
        ...
        triggerPanic: true,
        submitFieldReports: true, reviewFieldReports: true, matchMissingProfiles: true,
        dashboardType: 'admin',
      };
    case 'fleet_owner':
      return {
        ...
        triggerPanic: true,
        submitFieldReports: false, reviewFieldReports: false, matchMissingProfiles: false,
        dashboardType: 'fleet',
      };
    case 'helper':
      return {
    ...
    case 'verificador':
      // Nuevo rol del registro de hallazgos de campo. No responde a
      // incidentes vehiculares — hace trabajo forense de campo como
      // parte de un colectivo de búsqueda. Solo ve reportes de su team.
      return {
        viewAdminPanel: false, manageUsers: false, manageAllVehicles: false,
        viewOwnVehicles: false, manageGeofences: false, manageFleet: false,
        respondIncidents: false, viewGpsActivity: false, viewAlerts: false,
        triggerPanic: true,
        submitFieldReports: true, reviewFieldReports: false, matchMissingProfiles: false,
        dashboardType: 'verificador',
      };
    case 'citizen':
    default:
      return {
...
// ═══════════════════════════════════════════════════════════════════════════
// Registro de hallazgos de campo para colectivos de búsqueda.
// Ver docs/FIELD-REGISTRY.md y migración 023_field_registry.sql.
//
// Enforcement policy:
//   • Writes require role = admin | verificador
//   • Verificadores son team-scoped: solo ven/editan reportes de su team
//   • is_credential=TRUE media solo la ve admin; verificadores ven metadata
//   • profile_match + family_notified son admin-only (dignidad, cadena custodia)
//   • Audit log en toda mutación sensible
// ═══════════════════════════════════════════════════════════════════════════

// ── Helpers ────────────────────────────────────────────────────────────────

/** Lee el team_id del usuario desde la DB. Devuelve null si no tiene team. */
async function getUserTeamId(userId: string): Promise<string | null> {
  const r = await pool.query('SELECT team_id FROM users WHERE id = $1', [userId]);
  return r.rows[0]?.team_id ?? null;
}

/**
 * Verifica si el usuario tiene acceso al field_report. Admin siempre sí.
 * Verificador solo si el reporte es de su mismo team. Devuelve la fila del
 * reporte si tiene acceso, o null si no (ya respondió 403/404).
 */
async function requireFieldReportAccess(req: any, res: any, reportId: string): Promise<any | null> {
  const { userId, role } = req.user;
  const r = await pool.query(
    `SELECT id, reporter_user_id, team_id, latitude, longitude, accuracy_m,
            report_date, category, notes, terrain_poi_id,
            status, authority_notified_at, authority_notified_by,
            authority_reference, processed_at, processed_notes,
  ...
  if (!report) {
    res.status(404).json({ error: 'Field report not found' });
    return null;
  }
  ...
  const myTeam = await getUserTeamId(userId);
  if (!myTeam || myTeam !== report.team_id) {
    res.status(403).json({ error: 'Access denied' });
    return null;
  }
  return report;
...
api.post('/teams', authMiddleware, writeRateLimit, requireRole('admin'), asyncHandler(async (req, res) => {
  const { name, description, region, contact_email, contact_phone } = req.body;
  if (!name || typeof name !== 'string' || name.trim().length < 2 || name.length > 120) {
    res.status(400).json({ error: 'name requerido (2-120 caracteres)' });
    return;
  }
  const { userId } = (req as any).user;
  const r = await pool.query(
    `INSERT INTO teams (name, description, region, contact_email, contact_phone, created_by)
     VALUES ($1, $2, $3, $4, $5, $6)
     RETURNING id, name, description, region, contact_email, contact_phone,
               created_by, is_active, created_at, updated_at`,
    [name.trim(), description || null, region || null, contact_email || null, contact_phone || null, userId]
  );
  writeAuditLog(req, { action: 'team.create', targetType: 'team', targetId: r.rows[0].id, details: { name: r.rows[0].name } });
  res.json(r.rows[0]);
}));

api.get('/teams', authMiddleware, readRateLimit, requireRole('admin', 'verificador'), asyncHandler(async (req, res) => {
  const { userId, role } = (req as any).user;
  if (role === 'admin') {
    const r = await pool.query(
      `SELECT id, name, description, region, contact_email, contact_phone,
  ...
  // verificador: solo su propio team
  const myTeam = await getUserTeamId(userId);
...
api.get('/teams/:id', authMiddleware, readRateLimit, requireRole('admin', 'verificador'), asyncHandler(async (req, res) => {
  const { id } = req.params;
  if (!isValidUuid(id)) { res.status(400).json({ error: 'Invalid team ID' }); return; }
  const { userId, role } = (req as any).user;
  if (role !== 'admin') {
    const myTeam = await getUserTeamId(userId);
    if (myTeam !== id) { res.status(403).json({ error: 'Access denied' }); return; }
  }
  const r = await pool.query(
    `SELECT id, name, description, region, contact_email, contact_phone,
            is_active, created_at, updated_at
  ...
  if (!r.rows[0]) { res.status(404).json({ error: 'Team not found' }); return; }

  // Include team members (admins see encoded contact; verificadores only see names)
  const members = await pool.query(
    `SELECT id, name, role, created_at
     FROM users WHERE team_id = $1 ORDER BY created_at ASC`,
    [id]
  );
  res.json({ ...r.rows[0], members: members.rows });
}));

api.put('/teams/:id', authMiddleware, writeRateLimit, requireRole('admin'), asyncHandler(async (req, res) => {
  const { id } = req.params;
  if (!isValidUuid(id)) { res.status(400).json({ error: 'Invalid team ID' }); return; }
  const { name, description, region, contact_email, contact_phone, is_active } = req.body;
  const r = await pool.query(
    `UPDATE teams SET
       name = COALESCE($2, name),
     ...
     RETURNING id, name, description, region, contact_email, contact_phone,
               is_active, created_at, updated_at`,
    [id, name || null, description || null, region || null, contact_email || null, contact_phone || null,
     typeof is_active === 'boolean' ? is_active : null]
  );
  if (!r.rows[0]) { res.status(404).json({ error: 'Team not found' }); return; }
  writeAuditLog(req, { action: 'team.update', targetType: 'team', targetId: id });
  res.json(r.rows[0]);
}));

/** Asigna o remueve un usuario de un team. Body: { team_id: uuid | null }. */
api.put('/users/:id/team', authMiddleware, writeRateLimit, requireRole('admin'), asyncHandler(async (req, res) => {
  const { id } = req.params;
  if (!isValidUuid(id)) { res.status(400).json({ error: 'Invalid user ID' }); return; }
  const { team_id } = req.body;
  if (team_id !== null && !isValidUuid(team_id)) {
    res.status(400).json({ error: 'team_id debe ser UUID válido o null' });
    return;
  }
  if (team_id) {
    const t = await pool.query('SELECT 1 FROM teams WHERE id = $1 AND is_active = TRUE', [team_id]);
    if (!t.rows[0]) { res.status(400).json({ error: 'Team no existe o inactivo' }); return; }
  }
  const r = await pool.query(
  ...
  writeAuditLog(req, {
    action: team_id ? 'team.assign_user' : 'team.unassign_user',
    targetType: 'user',
    targetId: id,
    details: { team_id },
  });
...
api.post('/field-reports', authMiddleware, writeRateLimit, requireRole('admin', 'verificador'), asyncHandler(async (req, res) => {
  const { userId, role } = (req as any).user;
  const input = req.body;
  const err = validateFieldReportInput(input);
  if (err) { res.status(400).json({ error: err }); return; }

  // Verificadores deben pertenecer a un team; admin puede crear sin team.
  let teamId: string | null = null;
  if (role === 'verificador') {
    teamId = await getUserTeamId(userId);
    if (!teamId) {
      res.status(403).json({ error: 'Debes pertenecer a un team para crear reportes' });
      return;
    }
  } else if (role === 'admin') {
    // admin puede opcionalmente asociar el reporte a un team
    if (input.team_id) {
      if (!isValidUuid(input.team_id)) { res.status(400).json({ error: 'team_id inválido' }); return; }
      const t = await pool.query('SELECT 1 FROM teams WHERE id = $1', [input.team_id]);
      if (!t.rows[0]) { res.status(400).json({ error: 'team_id no existe' }); return; }
      teamId = input.team_id;
    }
  }

  // terrain_poi_id: si viene, validar que existe
  if (input.terrain_poi_id) {
    if (!isValidUuid(input.terrain_poi_id)) { res.status(400).json({ error: 'terrain_poi_id inválido' }); return; }
    const p = await pool.query('SELECT 1 FROM terrain_pois WHERE id = $1', [input.terrain_poi_id]);
    if (!p.rows[0]) { res.status(400).json({ error: 'terrain_poi_id no existe' }); return; }
  }

  const r = await pool.query(
    `INSERT INTO field_reports (
       reporter_user_id, team_id, latitude, longitude, geom, accuracy_m,
       report_date, category, notes, terrain_poi_id, status
     ) VALUES ($1, $2, $3, $4, ST_SetSRID(ST_MakePoint($4, $3), 4326), $5, $6, $7, $8, $9, 'pending')
     RETURNING id, reporter_user_id, team_id, latitude, longitude, accuracy_m,
               report_date, category, notes, terrain_poi_id, status,
               created_at, updated_at`,
    [userId, teamId, input.latitude, input.longitude, input.accuracy_m ?? null,
     input.report_date, input.category, input.notes || null, input.terrain_poi_id || null]
  );
  writeAuditLog(req, {
    action: 'field_report.create',
    targetType: 'field_report',
    targetId: r.rows[0].id,
    details: { category: input.category, team_id: teamId, has_poi: !!input.terrain_poi_id },
  });
  res.json(r.rows[0]);
}));

api.get('/field-reports', authMiddleware, readRateLimit, requireRole('admin', 'verificador'), asyncHandler(async (req, res) => {
  const { userId, role } = (req as any).user;
  const { status, category, team_id, limit } = req.query;
  ...
  if (role === 'verificador') {
    ...
    if (!myTeam) { res.json([]); return; }
    vals.push(myTeam);
    where.push(`team_id = $${vals.length}`);
  } else if (role === 'admin' && team_id) {
    if (!isValidUuid(String(team_id))) { res.status(400).json({ error: 'team_id inválido' }); return; }
    vals.push(team_id);
    where.push(`team_id = $${vals.length}`);
  }
  if (status) {
    if (!isValidFieldReportStatus(status)) { res.status(400).json({ error: 'status inválido' }); return; }
    vals.push(status);
  ...
  const r = await pool.query(
    `SELECT id, reporter_user_id, team_id, latitude, longitude, accuracy_m,
            report_date, category, notes, terrain_poi_id, status,
            authority_notified_at, authority_reference, processed_at,
            created_at, updated_at
...
api.get('/field-reports/:id', authMiddleware, readRateLimit, requireRole('admin', 'verificador'), asyncHandler(async (req, res) => {
  const { id } = req.params;
  if (!isValidUuid(id)) { res.status(400).json({ error: 'Invalid report ID' }); return; }
  const report = await requireFieldReportAccess(req, res, id);
  if (!report) return;
  res.json(report);
}));

api.put('/field-reports/:id/status', authMiddleware, writeRateLimit, requireRole('admin', 'verificador'), asyncHandler(async (req, res) => {
  const { id } = req.params;
  if (!isValidUuid(id)) { res.status(400).json({ error: 'Invalid report ID' }); return; }
  ...
  const { status, authority_reference, processed_notes } = req.body;
  if (!isValidFieldReportStatus(status)) {
    res.status(400).json({ error: 'status inválido' });
    return;
  }

  // Status transitions: pending → notified_authority → processed_by_authority → closed
  // Permitimos transiciones "hacia adelante" y a closed desde cualquiera.
  // Preferimos no restringir demasiado por ahora — el audit log preserva historial.
  const { userId } = (req as any).user;
  ...
  if (status === 'notified_authority') {
    vals.push(now); fields.push(`authority_notified_at = $${vals.length}`);
    vals.push(userId); fields.push(`authority_notified_by = $${vals.length}`);
    if (authority_reference) {
      vals.push(String(authority_reference).slice(0, 200));
      fields.push(`authority_reference = $${vals.length}`);
    }
  } else if (status === 'processed_by_authority') {
    vals.push(now); fields.push(`processed_at = $${vals.length}`);
    if (processed_notes) {
      vals.push(String(processed_notes).slice(0, 4000));
      fields.push(`processed_notes = $${vals.length}`);
    }
  }

  const r = await pool.query(
    `UPDATE field_reports SET ${fields.join(', ')} WHERE id = $1
     RETURNING id, status, authority_notified_at, authority_reference,
               processed_at, processed_notes, updated_at`,
    vals
  );
  writeAuditLog(req, {
    action: 'field_report.status',
    targetType: 'field_report',
    targetId: id,
...
api.delete('/field-reports/:id', authMiddleware, writeRateLimit, requireRole('admin'), asyncHandler(async (req, res) => {
  const { id } = req.params;
  if (!isValidUuid(id)) { res.status(400).json({ error: 'Invalid report ID' }); return; }
  const r = await pool.query('DELETE FROM field_reports WHERE id = $1 RETURNING id', [id]);
  if (!r.rows[0]) { res.status(404).json({ error: 'Field report not found' }); return; }
  writeAuditLog(req, { action: 'field_report.delete', targetType: 'field_report', targetId: id });
  res.json({ ok: true });
}));

// ── Field report media ─────────────────────────────────────────────────────

api.post('/field-reports/:id/media', authMiddleware, writeRateLimit, requireRole('admin', 'verificador'), asyncHandler(async (req, res) => {
  const { id } = req.params;
  if (!isValidUuid(id)) { res.status(400).json({ error: 'Invalid report ID' }); return; }
  const { userId } = (req as any).user;
  ...
  const { image_data, media_type, mime_type, is_credential, description } = req.body;
  if (typeof image_data !== 'string' || !image_data.length) {
    res.status(400).json({ error: 'image_data (base64) requerido' }); return;
  }
  if (!isWithinMediaSizeLimit(image_data)) {
    res.status(400).json({ error: 'image_data excede 1.2MB' }); return;
  }
  if (typeof is_credential !== 'boolean') {
    res.status(400).json({ error: 'is_credential (boolean) requerido — elige conscientemente si es credencial' });
    return;
  }
  const mt = media_type === 'video_frame' ? 'video_frame' : 'photo';
  const mime = (typeof mime_type === 'string' && mime_type.length <= 60) ? mime_type : 'image/jpeg';

  const r = await pool.query(
    `INSERT INTO field_report_media (field_report_id, media_type, image_data, mime_type, is_credential, description, uploaded_by)
     VALUES ($1, $2, $3, $4, $5, $6, $7)
     RETURNING id, field_report_id, media_type, mime_type, is_credential, description, uploaded_by, created_at`,
    [id, mt, image_data, mime, is_credential, description ? String(description).slice(0, 500) : null, userId]
  );
  writeAuditLog(req, {
    action: 'field_media.upload',
    targetType: 'field_report',
    targetId: id,
    details: { media_id: r.rows[0].id, is_credential, mime_type: mime },
  });
  res.json(r.rows[0]);
}));

api.get('/field-reports/:id/media', authMiddleware, readRateLimit, requireRole('admin', 'verificador'), asyncHandler(async (req, res) => {
  const { id } = req.params;
  if (!isValidUuid(id)) { res.status(400).json({ error: 'Invalid report ID' }); return; }
  ...
  const r = await pool.query(
    `SELECT id, field_report_id, media_type, image_data, mime_type,
            is_credential, description, uploaded_by, created_at
     FROM field_report_media WHERE field_report_id = $1 ORDER BY created_at ASC`,
    [id]
  );
  const isAdmin = role === 'admin';
  const safe = r.rows.map((row: FieldReportMedia) => redactCredentialMedia(row, isAdmin));

  // Si algún media fue redactado y el viewer no es admin, log para auditoría de acceso denegado
  const redactedCount = safe.filter((s: any) => s.redacted).length;
  if (redactedCount > 0 && !isAdmin) {
    writeAuditLog(req, {
      action: 'field_media.redacted_view',
      targetType: 'field_report',
      targetId: id,
      details: { redacted_count: redactedCount },
    });
  }
  res.json(safe);
}));

api.delete('/field-reports/:id/media/:mediaId', authMiddleware, writeRateLimit, requireRole('admin'), asyncHandler(async (req, res) => {
  const { id, mediaId } = req.params;
  if (!isValidUuid(id) || !isValidUuid(mediaId)) {
    res.status(400).json({ error: 'Invalid ID' }); return;
  }
  const r = await pool.query(
    `DELETE FROM field_report_media WHERE id = $1 AND field_report_id = $2 RETURNING id, is_credential`,
    [mediaId, id]
  );
  if (!r.rows[0]) { res.status(404).json({ error: 'Media not found' }); return; }
  writeAuditLog(req, {
    action: 'field_media.delete',
    targetType: 'field_media',
    targetId: mediaId,
    details: { field_report_id: id, was_credential: r.rows[0].is_credential },
  });
  res.json({ ok: true });
}));
...
api.post('/missing-persons', authMiddleware, writeRateLimit, asyncHandler(async (req, res) => {
  const { userId, role } = (req as any).user;
  const input = req.body;
  const err = validateMissingPersonInput(input);
  if (err) { res.status(400).json({ error: err }); return; }

  // Distinguir: admin creando en nombre de familia vs familia creando por sí misma
  const submitterField = role === 'admin' ? 'submitted_by_admin_id' : 'submitter_user_id';
  const isPublic = typeof input.is_public === 'boolean' ? input.is_public : true;

  const r = await pool.query(
    `INSERT INTO missing_persons_profiles (
       ${submitterField}, full_name, aliases, date_of_birth,
       disappearance_date, last_known_latitude, last_known_longitude,
       last_known_geom, last_known_description,
       physical_description, photo_base64,
       contact_email, contact_phone, contact_name, contact_relation,
       is_public, status
     ) VALUES (
       $1, $2, $3, $4, $5, $6, $7,
       CASE WHEN $6 IS NOT NULL AND $7 IS NOT NULL
         ...
         ELSE NULL END,
       $8, $9, $10, $11, $12, $13, $14, $15, 'searching'
     )
     RETURNING id, full_name, aliases, date_of_birth, disappearance_date,
               last_known_latitude, last_known_longitude, last_known_description,
               physical_description, photo_base64,
               contact_email, contact_phone, contact_name, contact_relation,
  ...
  writeAuditLog(req, {
    ...
    targetId: r.rows[0].id,
    details: { by_admin: role === 'admin', is_public: isPublic },
  });
  res.json(r.rows[0]);
}));

/**
 * GET /missing-persons — search / list.
 * - admin: ve todos
 * - verificador / otros: solo is_public=TRUE o los que son submitter
 * Query params: q (fuzzy name), status, limit
 */
api.get('/missing-persons', authMiddleware, readRateLimit, asyncHandler(async (req, res) => {
  const { userId, role } = (req as any).user;
  ...
  if (role !== 'admin') {
    vals.push(userId);
    where.push(`(is_public = TRUE OR submitter_user_id = $${vals.length})`);
  }
  if (q && typeof q === 'string' && q.trim().length >= 2) {
    vals.push(`%${q.trim()}%`);
    where.push(`full_name ILIKE $${vals.length}`);
  }
  ...
  vals.push(lim);
  const r = await pool.query(
    `SELECT id, full_name, aliases, date_of_birth, disappearance_date,
            last_known_latitude, last_known_longitude, last_known_description,
            physical_description,
            CASE WHEN $${vals.length} > 0 THEN photo_base64 ELSE NULL END AS photo_base64,
            contact_name, contact_relation,
            is_public, status, created_at, updated_at
     FROM missing_persons_profiles ${clause}
     ORDER BY created_at DESC
     LIMIT $${vals.length}`,
    vals
  );
...
api.get('/missing-persons/:id', authMiddleware, readRateLimit, asyncHandler(async (req, res) => {
  const { id } = req.params;
  if (!isValidUuid(id)) { res.status(400).json({ error: 'Invalid profile ID' }); return; }
  const { userId, role } = (req as any).user;

  const r = await pool.query(
    `SELECT id, submitter_user_id, submitted_by_admin_id,
            full_name, aliases, date_of_birth, disappearance_date,
            last_known_latitude, last_known_longitude, last_known_description,
            physical_description, photo_base64,
            contact_email, contact_phone, contact_name, contact_relation,
            is_public, status, status_updated_at, status_notes,
            created_at, updated_at
     FROM missing_persons_profiles WHERE id = $1`,
    [id]
  );
  const prof = r.rows[0];
  if (!prof) { res.status(404).json({ error: 'Profile not found' }); return; }
  if (role !== 'admin' && prof.submitter_user_id !== userId && !prof.is_public) {
    res.status(403).json({ error: 'Access denied' });
    return;
  }
  res.json(prof);
...
api.put('/missing-persons/:id', authMiddleware, writeRateLimit, asyncHandler(async (req, res) => {
  const { id } = req.params;
  if (!isValidUuid(id)) { res.status(400).json({ error: 'Invalid profile ID' }); return; }
  const { userId, role } = (req as any).user;

  const existing = await pool.query(
    `SELECT submitter_user_id FROM missing_persons_profiles WHERE id = $1`,
    [id]
  );
  if (!existing.rows[0]) { res.status(404).json({ error: 'Profile not found' }); return; }
  if (role !== 'admin' && existing.rows[0].submitter_user_id !== userId) {
    res.status(403).json({ error: 'Access denied' });
    return;
  }

  const input = req.body;
  // Reutilizamos el validador pero permitimos parciales: solo validamos
  // los campos efectivamente enviados.
  if (input.full_name !== undefined) {
    const err = validateMissingPersonInput({ full_name: input.full_name });
    if (err) { res.status(400).json({ error: err }); return; }
  }

  const r = await pool.query(
    `UPDATE missing_persons_profiles SET
       full_name = COALESCE($2, full_name),
       aliases = COALESCE($3, aliases),
       date_of_birth = COALESCE($4, date_of_birth),
       disappearance_date = COALESCE($5, disappearance_date),
       last_known_latitude = COALESCE($6, last_known_latitude),
       last_known_longitude = COALESCE($7, last_known_longitude),
       last_known_geom = CASE
         WHEN $6 IS NOT NULL AND $7 IS NOT NULL
         THEN ST_SetSRID(ST_MakePoint($7, $6), 4326)
         ELSE last_known_geom END,
       last_known_description = COALESCE($8, last_known_description),
       physical_description = COALESCE($9, physical_description),
       photo_base64 = COALESCE($10, photo_base64),
       contact_email = COALESCE($11, contact_email),
       contact_phone = COALESCE($12, contact_phone),
       contact_name = COALESCE($13, contact_name),
       contact_relation = COALESCE($14, contact_relation),
       is_public = COALESCE($15, is_public),
       updated_at = NOW()
     WHERE id = $1
     RETURNING id, full_name, status, is_public, updated_at`,
    [
      id,
      ...
      input.contact_name || null,
      input.contact_relation || null,
      typeof input.is_public === 'boolean' ? input.is_public : null,
    ]
  );
  writeAuditLog(req, { action: 'missing_person.update', targetType: 'missing_person', targetId: id });
  res.json(r.rows[0]);
}));

/** Status change: admin-only (dignidad — no la familia cambia a "found_deceased"). */
api.put('/missing-persons/:id/status', authMiddleware, writeRateLimit, requireRole('admin'), asyncHandler(async (req, res) => {
  const { id } = req.params;
  if (!isValidUuid(id)) { res.status(400).json({ error: 'Invalid profile ID' }); return; }
  const { userId } = (req as any).user;
  const { status, status_notes } = req.body;
  const valid = ['searching', 'found_deceased', 'found_alive', 'case_closed'];
  if (!valid.includes(status)) {
    res.status(400).json({ error: `status debe ser: ${valid.join(', ')}` });
    return;
  }
  const r = await pool.query(
    `UPDATE missing_persons_profiles SET
       status = $2, status_updated_at = NOW(), status_updated_by = $3,
       status_notes = $4, updated_at = NOW()
     WHERE id = $1
     RETURNING id, status, status_updated_at, status_notes`,
    [id, status, userId, status_notes ? String(status_notes).slice(0, 4000) : null]
  );
  if (!r.rows[0]) { res.status(404).json({ error: 'Profile not found' }); return; }
  writeAuditLog(req, {
    action: 'missing_person.status',
    targetType: 'missing_person',
    targetId: id,
    details: { status, has_notes: !!status_notes },
  });
  res.json(r.rows[0]);
}));

// ── Profile matches (admin-only, manual) ───────────────────────────────────

api.post('/profile-matches', authMiddleware, writeRateLimit, requireRole('admin'), asyncHandler(async (req, res) => {
  const { userId } = (req as any).user;
  const { field_report_id, missing_person_id, match_type, confidence, notes } = req.body;
  if (!isValidUuid(field_report_id) || !isValidUuid(missing_person_id)) {
    res.status(400).json({ error: 'field_report_id y missing_person_id deben ser UUIDs válidos' });
    return;
  }
  if (!isValidProfileMatchType(match_type)) {
    res.status(400).json({ error: 'match_type inválido' });
    return;
  }
  const conf = confidence && isValidProfileMatchConfidence(confidence) ? confidence : 'medium';

  // Verificar que ambos existen (prevenir orphan rows)
  const check = await pool.query(
    `SELECT
       (SELECT 1 FROM field_reports WHERE id = $1) AS fr,
       (SELECT 1 FROM missing_persons_profiles WHERE id = $2) AS mp`,
    [field_report_id, missing_person_id]
  );
  if (!check.rows[0].fr) { res.status(400).json({ error: 'field_report_id no existe' }); return; }
  if (!check.rows[0].mp) { res.status(400).json({ error: 'missing_person_id no existe' }); return; }

  const r = await pool.query(
    `INSERT INTO profile_matches (field_report_id, missing_person_id, matched_by, match_type, confidence, notes)
     VALUES ($1, $2, $3, $4, $5, $6)
     RETURNING id, field_report_id, missing_person_id, matched_by, match_type, confidence,
               notes, family_notified_at, created_at`,
    [field_report_id, missing_person_id, userId, match_type, conf, notes ? String(notes).slice(0, 4000) : null]
  );
  writeAuditLog(req, {
    action: 'profile_match.create',
    targetType: 'profile_match',
    targetId: r.rows[0].id,
    details: { field_report_id, missing_person_id, match_type, confidence: conf },
  });
  res.json(r.rows[0]);
}));

/** List matches. Admin ve todos; cualquier otro rol solo matches de reportes de su team. */
api.get('/profile-matches', authMiddleware, readRateLimit, requireRole('admin'), asyncHandler(async (req, res) => {
  const { field_report_id, missing_person_id, limit } = req.query;
  const lim = Math.min(parseInt(String(limit || '100'), 10) || 100, 500);
  const where: string[] = [];
  const vals: any[] = [];
  if (field_report_id) {
    if (!isValidUuid(String(field_report_id))) { res.status(400).json({ error: 'field_report_id inválido' }); return; }
    vals.push(field_report_id);
    where.push(`field_report_id = $${vals.length}`);
  }
  if (missing_person_id) {
    if (!isValidUuid(String(missing_person_id))) { res.status(400).json({ error: 'missing_person_id inválido' }); return; }
    vals.push(missing_person_id);
    where.push(`missing_person_id = $${vals.length}`);
  }
  const clause = where.length ? `WHERE ${where.join(' AND ')}` : '';
  vals.push(lim);
  const r = await pool.query(
    `SELECT id, field_report_id, missing_person_id, matched_by, match_type, confidence,
            notes, family_notified_at, family_notified_by, family_notification_channel,
            family_notification_notes, created_at
     FROM profile_matches ${clause}
     ORDER BY created_at DESC
     LIMIT $${vals.length}`,
    vals
  );
...
/**
 * Record family notification. SOLO se llama cuando un admin ya contactó
 * a la familia por un canal apropiado (teléfono/persona/vía-fiscalía).
 * Este endpoint NO notifica a nadie — solo documenta que ya se hizo.
 */
api.post('/profile-matches/:id/notify-family', authMiddleware, writeRateLimit, requireRole('admin'), asyncHandler(async (req, res) => {
  const { id } = req.params;
  if (!isValidUuid(id)) { res.status(400).json({ error: 'Invalid match ID' }); return; }
  const { userId } = (req as any).user;
  const { channel, notes } = req.body;
  if (!isValidNotificationChannel(channel)) {
    res.status(400).json({ error: 'channel inválido (phone|in_person|email_manual|via_authority)' });
    return;
  }
  const r = await pool.query(
    `UPDATE profile_matches SET
       family_notified_at = NOW(),
       family_notified_by = $2,
       family_notification_channel = $3,
       family_notification_notes = $4
     WHERE id = $1
     RETURNING id, family_notified_at, family_notification_channel`,
    [id, userId, channel, notes ? String(notes).slice(0, 4000) : null]
  );
  if (!r.rows[0]) { res.status(404).json({ error: 'Match not found' }); return; }
  writeAuditLog(req, {
    action: 'profile_match.notify_family',
    targetType: 'profile_match',
    targetId: id,
    details: { channel },
  });
